...
In some applications, we may want to relax the consistency contraint.  An application could be interested in specific values for the size of the community, internal and external density.  If this is the case, we suggest a polynomial approach to building a metric.
\begin{definition}[General Metric] The general metric for evaluating any single community is a sum of polynomial functions on internal and external density.
  \begin{equation}
...
We recommend that once your metric is built, you check the plots of level sets and size of community impact.  In particular, check that:
\begin{itemize}
\item The $(I, E)$ values that have local and global maximums correspond to communities you want to find.
\item That the metric aids finding those communities with level sets; the gradient of the level sets leads to the desired communities.  Again, this is only a visual check.  Graphs and networks are discrete.  The progression of an algorithm that maximizes the metric will not be able to follow the gradient in all cases, but it will stay as close as possible to the gradient.
\end{itemize}
  Given, our {\sc Linearity}$(C)$ metric, we now do just that.

\begin{figure}[!h]
\centering
\includegraphics[width=2.8in]{Figures/linear_single_ls}
\includegraphics[width=2.8in]{Figures/linear_single_k}
\caption{The two plots are boring and predictable.}
\label{fig:linear_single_ls_k}
\end{figure}


We now put our metric through the same analysis for the other single community metrics.  We chose our parameters $a$ and $b$ by setting $a=1$ and searching for $b$ that did not cause the greedy algorithm to return the entire graph but still had the greedy algorithm expand the initial community.  Cover a method to automatically find $a$ and $b$ in the next section.
JTODO
\begin{figure}[!h]
\centering
...
\caption{Our linearity metric is tested exactly as previous single community metrics have been with the Greedy Algorithm \ref{alg_greedy_single}.  Notice that the paths are truncated paths of Conductance in Figure \ref{fig:conductance}. JTODO add in all the final spots from previous metrics, also change b so that the final communities are more valued than the entire graph.}
...
\caption{Compared to Known Solutions:  For the karate club we find the two communities, but then expansion.  For the college football league we find 13 communities, rather than the real 12, but this results in a better value in internal and external density.  In the karate and football the black star corresponds to the value of known community solutions. }
...
\caption{Linearity is in red.  The $(I, E)$ values modularity produces are inserted with a black square In relativity there are no known communities.  For now we just show that good avlues of internal and external density can be found.}
