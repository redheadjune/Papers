...
While we have observed how metrics are affected by $I(C)$ and $E(C)$ values, we have not checked the last affect of the last variable that these metrics depended on.  For each network, we can consider $|V|$ to be a constant, but the size of the community does affect the metric evaluation.  The variable is $k$.

JTODO add plots of how $k$ affects the metrics.
